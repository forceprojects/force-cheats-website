...
const handleMoneyMotionCheckout = async (req, res) => {
  ...
  let upstream = null;
  try {
    upstream = await postUpstream(createUrl, { json: jsonPayload });
    if (upstream.status === 400 && looksLikeTrpcInvalidRequest(upstream)) {
      upstream = await postUpstream(createUrl, { input: jsonPayload });
    }
    if (upstream.status === 400 && looksLikeTrpcInvalidRequest(upstream)) {
      upstream = await postUpstream(createUrl, { json: jsonPayload, input: jsonPayload });
    }
    if (upstream.status === 400 && looksLikeTrpcInvalidRequest(upstream)) {
      const u = new URL(createUrl);
      if (!u.searchParams.has("batch")) u.searchParams.set("batch", "1");
      upstream = await postUpstream(u.toString(), [{ json: jsonPayload }]);
    }
    if (upstream.status === 400 && looksLikeTrpcInvalidRequest(upstream)) {
      upstream = await postUpstream(createUrl, { id: 0, json: jsonPayload });
    }
  } catch (e) {
    send(
      res,
